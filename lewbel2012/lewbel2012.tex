...
$W_{j}\triangleq Y_{j}-X\T(\E[XX\T])^{-1}\E[XY_{j}]$. A short calculation
gives 
\[
...
store the residuals $\hat{\varepsilon}_{2}=Y_{2}-X\T\hat{\beta}_{2}^{\text{OLS}}$.
...
The parameter vector is $\theta=(\beta_{1}\T,\gamma_{1},\beta_{2}\T)\T$.
The moment vector is formed by multiplying instruments by the model's
structural errors, which are defined as $\varepsilon_{1}(\theta)=Y_{1}-X\T\beta_{1}-Y_{2}\gamma_{1}$
and $\varepsilon_{2}(\theta)=Y_{2}-X\T\beta_{2}$. 
\begin{equation}
...
The parameter vector is $\theta=(\beta_{1}\T,\gamma_{1},\beta_{2}\T,\gamma_{2})\T$.
The structural errors are now $\varepsilon_{1}(\theta)=Y_{1}-X\T\beta_{1}-Y_{2}\gamma_{1}$
and $\varepsilon_{2}(\theta)=Y_{2}-X\T\beta_{2}-Y_{1}\gamma_{2}$.
\begin{equation}
...
\begin{itemize}
\itemsep0pt
\item The parameter $\beta_{2}$ is always point-identified by OLS: $\hat{\beta}_{2}=(\E[X\T X])^{-1}\E[X\T Y_{2}]$. 
...
by $\beta_{1,k}=(\E[X\T X])^{-1}\E[X\T(Y_{1}-Y_{2}\gamma_{1,k})]$. 
\end{itemize}
...
\begin{enumerate}
\itemsep2pt
\item Estimate the second equation by OLS to get residuals $\hat{\varepsilon}_{2t}=Y_{2t}-X_{t}\T\hat{\beta}_{2}$. 
...
\[
\log\sigma_{jt}^{2}=X_{t}\T\delta_{j},\quad\text{where}\quad\varepsilon_{jt}\mid\mathcal{F}_{t-1}\sim(0,\sigma_{jt}^{2})\quad\text{for }j=1,2.
\]
...
The full set of parameters is $\theta=(\beta_{1}\T,\beta_{2}\T,\gamma_{1},\gamma_{2},\delta_{1}\T,\delta_{2}\T)\T$.
...
and the structural errors (e.g., $\varepsilon_{1t}(\theta)=Y_{1t}-X_{t}\T\beta_{1}-Y_{2t}\gamma_{1}$),
