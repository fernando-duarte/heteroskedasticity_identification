...
plot(data$time, sqrt(data$sigma2_sq), type = "l",
...
```{r sensitivity-garch, cache=TRUE}
...
For full functionality, install the `rugarch` package:

```{r eval=FALSE}
install.packages("rugarch")
```

Without `rugarch`, the method falls back to using squared residuals as a proxy for conditional variance, which is less efficient.
...
```{r prono-replication, cache=TRUE}
...
run_prono_replication <- function(rho, n_sims = 1000) {
  ...
  summary_stats <- data.frame(
    ...
    RMSE = c(sqrt(mean(results_df$ols_bias^2)), 
...
p_replication <- ggplot(plot_data, aes(x = Estimate, fill = Method)) +
  ...
  labs(title = "Replication of Prono (2014) Results",
...
f_stats_data <- data.frame(
  Correlation = c(rep("Low (ρ = 0.20)", nrow(results_low$results)),
...
p_fstats <- ggplot(f_stats_data, aes(x = F_stat, fill = Correlation)) +
  ...
  labs(title = "First-Stage F-Statistics in Replication",
       subtitle = "Weak instrument threshold = 10 (red line)",
       x = "F-statistic",
       y = "Count") +
  theme_minimal() +
  scale_fill_manual(values = c("Low (ρ = 0.20)" = "#3498DB", 
                               "High (ρ = 0.40)" = "#E74C3C"))
...
FFdownload(output_file = tempf, 
           inputlist = inputlist, 
           exclude_daily = FALSE)  # We want weekly data
```
...
run_prono_asset_pricing <- function(data, window_size = 260) {
  ...
  for (i in window_size:nrow(data)) {
    ...
    tryCatch({
      ...
      # Fit GARCH if rugarch available
      if (requireNamespace("rugarch", quietly = TRUE)) {
        spec <- rugarch::ugarchspec(
          variance.model = list(model = "sGARCH", garchOrder = c(1, 1)),
          mean.model = list(armaOrder = c(0, 0), include.mean = FALSE)
        )
        garch_fit <- rugarch::ugarchfit(spec = spec, data = e2)
