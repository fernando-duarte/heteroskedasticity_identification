...
```{r monte-carlo, eval=FALSE, fig.alt="Density plot comparing the sampling distributions of OLS and Rigobon estimators across Monte Carlo simulations, with OLS shown in one color and Rigobon in another, and the true parameter value marked as a vertical red dashed line"}
...
# Example showing potential coefficient of variation for estimates if run in MC
# This is a conceptual calculation
# Example data: alpha_estimates <- rnorm(100, mean = 0.6, sd = 0.1) # from MC
# beta_estimates <- rnorm(100, mean = 0.2, sd = 0.05)
# if (exists("alpha_estimates") && exists("beta_estimates")) {
# cat("CV for alpha estimates: ", round(sd(alpha_estimates) / mean(alpha_estimates), 3), "\n")
# cat("CV for beta estimates: ", round(sd(beta_estimates) / mean(beta_estimates), 3), "\n")
# }
