...
_Time-series note:_ in Section \@ref(time-series-variant) we weaken this to a martingale-difference assumption.
...
5. Proceed with 2SLS as in Section \@ref(feasible-2sls), using $[X_t, IV_t]$ as instruments for $Y_{2t}$ in the first structural equation. Use HAC-robust standard errors.
...
X_t \cdot \varepsilon_{1t}(\theta) \\[2pt]
X_t \cdot \varepsilon_{2t}(\theta) \\[2pt]
Z_t \cdot \varepsilon_{1t}(\theta)\varepsilon_{2t}(\theta) \\[2pt]
Z_t \cdot (\varepsilon_{1t}(\theta)^2 - e^{X_t^\top\delta_1}) \\[2pt]
...
Therefore, the sample average of $Q_t(\theta)$ is a computable criterion function for any candidate parameter values. As in Section \@ref(simultaneous-system), $Z_t$ must have at least 2 entries (including perhaps a constant, if Corollary 1 applies).
