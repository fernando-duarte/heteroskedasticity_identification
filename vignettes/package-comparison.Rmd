...
test_data$rendo_iv_manual <- rendo_iv_manual  # Add to dataframe for later use
...
# REndo might use X1 and generated_IV as instruments for P
# Note: P cannot be both endogenous and an instrument
overid_model <- tryCatch(
  {
    ivreg(y ~ X1 + P | X1 + lewbel_iv + rendo_iv_manual, data = test_data)
  },
  error = function(e) {
    cat("   Could not estimate overidentified model: ", e$message, "\n")
    NULL
  }
)
