...
get_stata_path <- function() {
  ...
  for (path in stata_paths) {
    ...
    if (nzchar(full_path)) return(as.character(full_path))
  }
...
ensure_stata_packages <- function() {
  ...
  if (is.null(stata_path)) return(FALSE)
...
generate_hetid_test_data <- function(n = 1000, seed = 42) {
  ...
  data$lewbel_iv <- data$lewbel_iv - mean(data$lewbel_iv) # ensure exactly mean zero
