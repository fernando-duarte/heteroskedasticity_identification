...
verify_klein_vella_assumptions <- function(data, config = NULL, verbose = TRUE) {
  ...
  exog_test_1 <- lm(e1 ~ ., data = data[, x_vars, drop = FALSE])
  exog_test_2 <- lm(e2 ~ ., data = data[, x_vars, drop = FALSE])

  results$exogeneity <- list(
    p_value_y1 = summary(exog_test_1)$fstatistic,
    p_value_y2 = summary(exog_test_2)$fstatistic,
