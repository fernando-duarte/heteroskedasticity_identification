...
plot_estimator_distributions <- function(results_clean, config) {
  ...
  plot_data <- dplyr::select(results_clean, .data$ols_gamma1, .data$tsls_gamma1)
  ...
  ggplot2::ggplot(
    ...
    ggplot2::geom_vline(
      ...
      linetype = "dashed",
      color = "red",
      linewidth = constants_env$PLOT_LINE_WIDTH_THIN
    ) +
    ...
    ggplot2::theme_minimal(base_size = constants_env$PLOT_BASE_FONT_SIZE) +
...
plot_first_stage_f_dist <- function(results_clean,
  ...
  if (is.null(weak_iv_pct)) {
    ...
    if ("first_stage_F" %in% names(results_clean)) {
      weak_iv_pct <- mean(results_clean$first_stage_F < constants_env$WEAK_INSTRUMENT_F_THRESHOLD, na.rm = TRUE) * 100
    } else {
      weak_iv_pct <- 0
    }
  }
  ...
  ggplot2::ggplot(results_clean, ggplot2::aes(x = .data$first_stage_F)) +
    ggplot2::geom_histogram(bins = constants_env$PLOT_HISTOGRAM_BINS, fill = "steelblue", alpha = 0.7) +
    ggplot2::geom_vline(xintercept = constants_env$WEAK_INSTRUMENT_F_THRESHOLD, linetype = "dashed", color = "red") +
    ggplot2::annotate("text",
      x = constants_env$WEAK_INSTRUMENT_F_THRESHOLD, y = Inf, label = paste("F =", constants_env$WEAK_INSTRUMENT_F_THRESHOLD),
      color = "red", angle = 90, vjust = 1.5, hjust = -0.5
    ) +
    ggplot2::labs(
      title = "Distribution of First-Stage F-Statistics",
      subtitle = sprintf(
        paste0("%.1f%% have F < ", constants_env$WEAK_INSTRUMENT_F_THRESHOLD, " (weak instrument threshold)"),
        weak_iv_pct
      ),
...
plot_bootstrap_ci <- function(bootstrap_examples, config) {
  if (nrow(bootstrap_examples) < constants_env$PLOT_MIN_BOOTSTRAP_THRESHOLD) {
    return(NULL)
  }

  # Prepare data
  plot_data <- dplyr::slice_head(bootstrap_examples, n = constants_env$PLOT_BOOTSTRAP_DISPLAY_LIMIT)
  ...
  ggplot2::ggplot(plot_data) +
    ggplot2::geom_segment(
      ggplot2::aes(
        x = .data$bound_lower_tau_set - constants_env$Z_CRITICAL_95 * .data$bound_se_lower,
        xend = .data$bound_upper_tau_set + constants_env$Z_CRITICAL_95 * .data$bound_se_upper,
        y = .data$sim_id_ordered, yend = .data$sim_id_ordered
      ),
      color = "lightgray", linewidth = constants_env$PLOT_LINE_WIDTH_THICK, alpha = 0.5
    ) +
    ggplot2::geom_segment(
      ggplot2::aes(
      ...
      color = "steelblue", linewidth = constants_env$PLOT_LINE_WIDTH_NORMAL
    ) +
    ggplot2::geom_vline(
