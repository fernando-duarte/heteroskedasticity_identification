...
plot_estimator_distributions <- function(results_clean, config) {
  results_clean %>%
    dplyr::select(ols_gamma1, tsls_gamma1) %>%
    tidyr::pivot_longer(cols = dplyr::everything(), names_to = "Estimator", values_to = "Estimate") %>%
    dplyr::mutate(Estimator = dplyr::recode(Estimator,
                                           "ols_gamma1" = "OLS (Biased)",
                                           "tsls_gamma1" = "2SLS (Lewbel)")) %>%
    ggplot2::ggplot(ggplot2::aes(x = Estimate, fill = Estimator)) +
    ggplot2::geom_density(alpha = 0.7) +
    ggplot2::geom_vline(xintercept = config$gamma1, linetype = "dashed", color = "red", linewidth = 1) +
    ggplot2::annotate("text", x = config$gamma1, y = Inf, label = paste("True =", config$gamma1), 
...
plot_sample_size_consistency <- function(results_by_n, config) {
  results_by_n %>%
    dplyr::filter(!is.na(tsls_gamma1)) %>%
    ggplot2::ggplot(ggplot2::aes(x = factor(sample_size), y = tsls_gamma1)) +
...
#'
#' @export
plot_heteroscedasticity_sensitivity <- function(results_by_delta, config) {
  results_by_delta %>%
    dplyr::filter(!is.na(tsls_gamma1)) %>%
    ggplot2::ggplot(ggplot2::aes(x = factor(delta_het), y = tsls_gamma1)) +
...
#'
#' @export
plot_first_stage_f_distribution <- function(results_clean, weak_iv_pct) {
  results_clean %>%
    ggplot2::ggplot(ggplot2::aes(x = first_stage_F)) +
    ggplot2::geom_histogram(bins = 50, fill = "steelblue", alpha = 0.7) +
    ggplot2::geom_vline(xintercept = 10, linetype = "dashed", color = "red") +
    ggplot2::annotate("text", x = 10, y = Inf, label = "F = 10", 
...
#'
#' @export
plot_bootstrap_confidence_intervals <- function(bootstrap_examples, config) {
  ...
  bootstrap_examples %>%
    ...
    dplyr::mutate(sim_id_ordered = dplyr::row_number()) %>%
    ggplot2::ggplot() +
    ggplot2::geom_segment(ggplot2::aes(x = bound_lower_tau_set - 1.96 * bound_se_lower, 
                                      xend = bound_upper_tau_set + 1.96 * bound_se_upper,
                                      y = sim_id_ordered, yend = sim_id_ordered), 
                         color = "lightgray", linewidth = 3, alpha = 0.5) +
    ggplot2::geom_segment(ggplot2::aes(x = bound_lower_tau_set, xend = bound_upper_tau_set,
                                      y = sim_id_ordered, yend = sim_id_ordered), 
...
#' bootstrap_examples <- analyze_bootstrap_results(results_main, bootstrap_demo, config)
#' 
#' plots <- generate_all_plots(results_main, results_by_n, results_by_delta, 
...
generate_all_plots <- function(results_main, results_by_n, results_by_delta, 
  ...
  plots <- list(
    ...
    sensitivity = plot_heteroscedasticity_sensitivity(results_by_delta, config),
    first_stage_f = plot_first_stage_f_distribution(results_clean, weak_iv_pct),
    bootstrap_ci = plot_bootstrap_confidence_intervals(bootstrap_examples, config)
  )
