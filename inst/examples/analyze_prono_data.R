...
# Read the data
data_file <- "/Users/fernandoduarte/Dropbox (Personal)/Research/vfci/V-FCI-Overleaf/heteroskedasticity/lewbel2012/tp-files/ff_sze_bm_ind_Prono2013.txt"
...
cat(sprintf("   Skewness: %.3f\n", (sum((market_excess - mean(market_excess))^3)/length(market_excess))/(sd(market_excess)^3)))
cat(sprintf("   Kurtosis: %.3f\n\n", (sum((market_excess - mean(market_excess))^4)/length(market_excess))/(sd(market_excess)^4)))
